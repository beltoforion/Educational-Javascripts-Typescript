...
export class TidalSimulation {
    ...
    onMouseDown(evt) {
        ...
        let dist = Vec2d.subtractEx(this.moon.pos, clickPos).length();
    ...
    onMouseMove(evt) {
        ...
        x /= this.scaleDist;
        y /= this.scaleDist;
        let newMoonPos = new Vec2d(x, y);
        let vecEarthMoon = Vec2d.subtractEx(newMoonPos, this.earth.pos);
        let dist = Vec2d.subtractEx(this.earth.pos, newMoonPos).length();
    ...
    */
    ...
    update() {
        ...
        for (let i = 1; i < this.numArrows + 1; ++i) {
            let posSurface = new Vec2d(Math.sin(i * delta) * this.earth.r * scaleSize, Math.cos(i * delta) * this.earth.r * scaleSize);
            //
    ...
    renderSun() {
        ...
        let vecBeamOrtho = new Vec2d(vecBeam.y, -vecBeam.x).multiplyValue(this.earth.r * this.scaleSize);
    ...
    renderEarth() {
        ...
        if (this.config.showGravAcc ||
            ...
            for (let i = 1; i < this.numArrows + 1; ++i) {
                ...
                let posScreen = Vec2d.addEx(posEarthScreen, new Vec2d(Math.sin(i * delta) * this.earth.r * this.scaleSize, Math.cos(i * delta) * this.earth.r * this.scaleSize));
                //
                // Tidal force Moon
                //
    ...
    renderSurfacePoints() {
        ...
        let posEarthScreen = this.mapToScreen(this.earth.pos, this.scaleSize);
        let orig = new Vec2d(0, this.earth.r * this.scaleSize);
